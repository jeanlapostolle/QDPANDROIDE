...
class MLP:
    ...
    def __init__(self, nInput, nHidden, nOutput):
        self.WList = [np.random.rand(
            nHidden, nInput + 1), np.random.rand(nOutput, nHidden + 1)]
    ...
    def compute(self, inputValues):
        ...
        print(self.WList)
        self.lastHiddenOutput = np.dot(
            self.WList[0], np.transpose(inputValues))
...
class Backpropagation:
    ...
    def __init__(self, mlp, alpha, regularization):
        self.DeltaList = [np.zeros(
            (mlp.nHidden, mlp.nInput + 1)), np.zeros((mlp.nOutput, mlp.nHidden + 1))]
    ...
    def iterate(self, inputValues, outputValues):
        for i in range(len(inputValues)):
            ...
            gz2 = np.multiply(self.mlp.lastHiddenOutput,
                              (1 - self.mlp.lastHiddenOutput))
            delta2 = np.multiply(Od3, gz2)

            self.DeltaList[1] = np.add(self.DeltaList[1], np.outer(
                delta3, self.mlp.lastHiddenOutput))
            self.DeltaList[0] = np.add(self.DeltaList[0], np.delete(
                np.outer(delta2, self.mlp.lastInput), 0, 0))

        self.DeltaList[0] = self.DeltaList[0] / \
            len(inputValues) + self.regularization * self.mlp.WList[0]
        self.DeltaList[1] = self.DeltaList[1] / \
            len(inputValues) + self.regularization * self.mlp.WList[1]
